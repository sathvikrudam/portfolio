...
const stats = [
  ...
  { value: "1000+", label: "Coding Problems Solved Across Platforms" },
...
const AboutSection = () => {
  ...
  return (
    ...
    >
      ...
      <div
        ref={ref}
        className="container mx-auto px-5 sm:px-6 relative z-10 w-full max-w-full overflow-hidden"
      >
        {/* SECTION HEADER */}
        <div
        ...
        <div className="grid lg:grid-cols-5 gap-16">
          ...
          >
            ...
            <div className="bg-card border border-border rounded-2xl overflow-hidden transition-all duration-500 hover:-translate-y-1 hover:shadow-xl hover:border-muted-foreground/30">
              ...
              <div className="p-6 space-y-4 text-sm leading-relaxed text-muted-foreground">
                <p>
                  ...
                  currently pursuing my B.Tech degree at{" "}
            ...
            {/* 
              ...
              This prevents horizontal overflow on phones.
            */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 w-full">
              {stats.map((stat, index) => (
                <div
                  key={stat.label}
                  className={`group min-w-0 w-full bg-card border border-border rounded-xl p-4 sm:p-5 text-center transition-all duration-500 hover:-translate-y-2 hover:shadow-lg hover:border-muted-foreground/40 ${
                    isVisible
                      ? "opacity-100 translate-y-0"
                ...
                >
                  <p className="font-display text-2xl sm:text-3xl font-black text-foreground group-hover:scale-110 transition-transform duration-300">
                    {stat.value}
                  </p>

                  <p className="text-[11px] sm:text-xs font-mono text-muted-foreground mt-2 leading-tight break-words">
                    {stat.label}
                  </p>
                </div>
              ))}
          ...
          >
            ...
            {focusAreas.map(({ icon: Icon, label, desc }) => (
              ...
              >
                {/* ICON */}
                <div className="w-10 h-10 shrink-0 rounded-lg bg-muted/50 flex items-center justify-center group-hover:bg-muted transition-all duration-300">
                  <Icon
                    size={18}
                ...
                {/* GLOW BORDER */}
                <div className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-100 transition duration-500 border border-muted-foreground/20 pointer-events-none" />
              </div>
            ))}
          </div>
        </div>
